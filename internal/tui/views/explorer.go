...
func (v ExplorerView) Update(msg tea.Msg, c *client.Client) (ExplorerView, tea.Cmd) {
	...
	if v.clusterList.IsActive() {
		var cmd tea.Cmd
		v.clusterList, cmd = v.clusterList.Update(msg, c)
		return v, cmd
	}
...
func (v ExplorerView) handleNavigation(msg tea.Msg, cmd tea.Cmd, c *client.Client) (ExplorerView, tea.Cmd) {
	...
	case "ctrl+r":
		if !v.clusterList.IsActive() && !v.queryList.IsActive() && !v.queryBar.IsActive() {
			var activateCmd tea.Cmd
			v.clusterList, activateCmd = v.clusterList.Activate(c)
			return v, activateCmd
		}
	case "/":
		if v.active == paneSidebar && v.viewMode == viewModeFull {
